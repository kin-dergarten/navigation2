  ...
  const pf_vector_t & delta)
{
  ...
  // similar to OdometryUpdate but we are only sampling form noise distribution
  // without using delta for an actual motion estimate
  void
  DifferentialMotionModel::noiseOnlyUpdate(
   pf_t * pf, const pf_vector_t & pose,
   const pf_vector_t & delta)
  {
    std::cout << "NoiseOnlyUpdate called with delta " << delta.v[0] << " " << delta.v[1] << " "  << delta.v[2] << std::endl;
    std::cout << "alpha1 " << alpha1_ << " alpha2 " << alpha2_ << " alpha3 "  << alpha3_ << " alpha4 " << alpha4_ << std::endl;
    ...
    delta_trans = sqrt(
     ...
     delta.v[1] * delta.v[1]);
    delta_rot2 = angleutils::angle_diff(delta.v[2], delta_rot1);
    std::cout << "calculated delta_rot1 " << delta_rot1 << " delta_rot2 " << delta_rot2 << " delta_trans "  << delta_trans << std::endl;
      ...
      delta_rot1_hat = angleutils::normalize(
       pf_ran_gaussian(
        sqrt(
         alpha1_ * delta_rot1_noise * delta_rot1_noise +
         alpha2_ * delta_trans * delta_trans)));
      delta_trans_hat = pf_ran_gaussian(
                         sqrt(
                          alpha3_ * delta_trans * delta_trans +
                          alpha4_ * delta_rot1_noise * delta_rot1_noise +
                          alpha4_ * delta_rot2_noise * delta_rot2_noise));
